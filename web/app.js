...
const contactYearFilterSection = document.getElementById('contact-year-filter-section');
...
function renderContactYearFilter(years) {
  ...
  contactYearFilterGrid.innerHTML = '<button data-year="all" class="active">All time</button>' +
...
function renderContacts(filter = '') {
  ...
  contactList.innerHTML = filtered.map(c => {
    ...
    const metaText = isPending ? 'Processing...' : `${formatNumber(c.total)} messages`;
...
function getAnalysisYears(analysis) {
  ...
  for (const [key, data] of Object.entries(analysis || {})) {
    if (typeof data === 'object' && data !== null) {
      const years = Object.keys(data).filter(k => k !== 'all' && /^\d{4}$/.test(k));
      if (years.length > 0) {
...
function renderAnalysisYearFilter(years) {
  ...
  analysisYearFilterGrid.innerHTML = '<button data-year="all" class="active">All time</button>' +
    years.map(y => `<button data-year="${y}">${y}</button>`).join('');
...
function renderAnalysis(analysis, contactName) {
  ...
  if (analysis.temperature) {
    renderTemperature(analysis.temperature, contactName, currentContactYearFilter);
...
function renderLinks(linksData, contactName, year = 'all') {
  ...
  if (sortedDomains.length > 0) {
    topDomainsEl.innerHTML = sortedDomains.map(d => {
...
function renderKeywords(keywordsData, contactName, year = 'all') {
  ...
  if (sentTopics.length > 0) {
    sentEl.innerHTML = sentTopics.slice(0, 6).map(k =>
      `<span class="keyword-tag sent-keyword">${getLabel(k)}</span>`
    ).join('');
  } else {
  ...
  if (receivedTopics.length > 0) {
    receivedEl.innerHTML = receivedTopics.slice(0, 6).map(k =>
...
function renderLlmThemes(themesData, year = 'all') {
  ...
  // Render theme tags
  if (themes.length > 0) {
    tagsEl.innerHTML = themes.map(theme =>
...
function renderAttachmentDetails(att, links) {
  ...
  if (totalLinks) totalParts.push(`${formatNumber(totalLinks)}${nbsp}link${totalLinks === 1 ? '' : 's'}`);
  ...
  if (links?.links_sent) sentParts.push(`${formatNumber(links.links_sent)}${nbsp}link${links.links_sent === 1 ? '' : 's'}`);
  ...
  if (links?.links_received) recvParts.push(`${formatNumber(links.links_received)}${nbsp}link${links.links_received === 1 ? '' : 's'}`);
...
function renderChart(monthlyData) {
  ...
  const currentMonthlyData = monthlyData || (currentContactData && currentContactData.monthly);
...
function loadEveryoneYear(year) {
  ...
  if (year === 'all') {
    ...
    renderAttachmentDetails(everyoneData.by_year[year].attachments, everyoneData.by_year[year].links);
  }
...
document.addEventListener('keydown', (e) => {
  ...
  if ((e.key === 'ArrowLeft' || e.key === 'ArrowRight') && !isEveryoneView && currentContactData) {
    const years = getYearsFromMonthly(currentContactData.monthly);
...
toggleButtons.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (!isEveryoneView && currentContactData) {
      const filteredMonthly = filterMonthlyByYear(currentContactData.monthly, currentContactYearFilter);
      renderChart(filteredMonthly);
    } else {
      renderChart();
    }
...
function downloadScreenshot() {
  ...
  link.download = `imessage-stats-${name.toLowerCase().replace(/\s+/g, '-')}.png`;
...
document.getElementById('copy-screenshot').addEventListener('click', copyScreenshot);
...
function renderLinksList(links) {
  ...
  linksList.innerHTML = links.map(({ url, direction }) => `
    <li>
      <a href="${formatLinkUrl(url)}" target="_blank" rel="noopener noreferrer" class="link-item" title="${url}">
...
linksSearchInput.addEventListener('input', (e) => {
  ...
  const filtered = allLinksForSearch.filter(({ url }) =>
...
async function checkLlmStatus() {
  try {
    const response = await fetch('data/_llm_status.json', { cache: 'no-store' });
    if (response.ok) {
...
function startLlmPolling() {
  ...
  llmPollingInterval = setInterval(async () => {
    ...
    } else if (stillRunning && currentContactFilename && !isContactPending(currentContactFilename)) {
...
async function reloadCurrentContact() {
  ...
  try {
    const response = await fetch(`data/messages/${currentContactFilename}.json`, { cache: 'no-store' });
    if (response.ok) {
